...
func printFileNormal(bicepFile *types.BicepFile, filename string, outdated bool) {
	fmt.Printf("\n%s:\n", filename)
...
func printDirectoryNormal(bicepDirectory *types.BicepDirectory, outdated bool) {
	fmt.Printf("\n%s:\n", bicepDirectory.Name)
	for _, file := range bicepDirectory.Files {
		filename, err := filepath.Rel(path, file.Name)
		if err != nil {
			filename = file.Name
		}
		printFileNormal(&file, filename, outdated)
	}
}
